					...
					struct v4l2_capability *v)
{
	...
	strlcpy(v->driver, videodev->dev.driver->name, sizeof(v->driver));
	strlcpy(v->card, videodev->name, sizeof(v->card));
			    ...
			    struct v4l2_control *ctrl)
{
    ...
    dev_dbg(&radio->client->dev, "%s ctrl id: 0x%x\n", __func__, ctrl->id);
	switch (ctrl->id) {
...
int si4730_fops_release(struct file *file)
{
	...
	/* stop radio */
	si4730_set_power_state(radio, POWER_OFF);
	radio->users = 0;
	return retval;
}
